...
int pb::init()
{
	...
	auto useBmpFont = 0;
	pinball::get_rc_int(IDS_STRING258, &useBmpFont);
	if (useBmpFont)
...
void pb::SelectDatFile(const std::vector<const char*>& dataSearchPaths)
{
	...
	{
		"CADET.DAT",
		options::get_string("Pinball Data", pinball::get_rc_string(IDS_STRING268, 0)),
		"DEMO.DAT",
	};
...
void pb::toggle_demo()
{
	if (demo_mode)
	{
		...
		auto text = pinball::get_rc_string(IDS_STRING125, 0);
...
void pb::frame(float dtMilliSec)
{
	...
	if (!MainTable->TiltLockFlag)
	{
		...
		{
			pinball::InfoTextBox->Display(pinball::get_rc_string(IDS_STRING126, 0), 2.0);
		}
...
void pb::pause_continue()
{
	...
	if (winmain::single_step)
	{
		...
		pinball::InfoTextBox->Display(pinball::get_rc_string(IDS_STRING123, 0), -1.0);
		midi::music_stop();
		Sound::Deactivate();
	}
	else
	{
		...
		if (!demo_mode)
		{
			...
			{
				textTime = -1.0;
				text = pinball::get_rc_string(IDS_STRING125, 0);
			}
			else
			{
				textTime = 5.0;
				text = pinball::get_rc_string(IDS_STRING124, 0);
			}
...
void pb::InputDown(GameInput input)
{
	...
	if (cheat_mode && input.Type == InputTypes::Keyboard)
	{
		...
		case 'h':
		{
			...
			strncpy(entry.Name, pinball::get_rc_string(IDS_STRING127, 0), sizeof entry.Name - 1);
	...
	if (!demo_mode && !MainTable->CheatsUsed)
	{
		for (auto i = 0; i < playerCount; ++i)
		{
			...
			if (position >= 0)
			{
				...
				strncpy(entry.Name, pinball::get_rc_string(scoreIndex[i] + IDS_STRING127, 0), sizeof entry.Name - 1);
				high_score::show_and_set_high_score_dialog({ entry, -1 });
			}
		}
	}
